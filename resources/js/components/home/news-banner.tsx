        ...
        <section className="bg-emerald-50 py-3">
            <div className="mx-auto flex max-w-6xl flex-col items-center justify-between px-4 md:flex-row">
                <div className="flex w-full items-center">
                    <span className="mr-3 rounded-md bg-emerald-600 px-2 py-1 text-xs font-bold whitespace-nowrap text-white">最新消息</span>
                    <div className="relative h-6 w-full overflow-hidden">
                        {news.map((item, index) => (
                            ...
                            >
                                <Link href={route('news.show', { slug: item.slug })} className="block truncate hover:text-emerald-600">
                                    {item.title}
                                </Link>
                            </div>
                        ))}
                ...
                <Link href={route('news.index')} className="text-sm whitespace-nowrap text-emerald-600 hover:underline">
                    查看全部
                </Link>
            </div>
        </section>
